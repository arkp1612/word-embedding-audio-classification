...
def dict_generator():
    req = urllib.request.Request('https://raw.githubusercontent.com/arkp1612/word-embedding-audio-classification/main/EDA%20and%20Data%20Input%20Prep/mtt_encodings.txt')
...
def generate_tag_num_dict():
    ...
    all_tags = ['classical','slow','techno','electronic','rock','fast','ambient','beat','vocal','synth','female',
    'male','singing','no vocals','loud','quiet','soft','new age','dance','weird','choir','trance','heavy',
    'upbeat','funky']
...
def _generate_datasets(tfrecords, audio_format, split=None, which_split=None, sample_rate=16000, num_mels=128, batch_size=32, block_length=1, cycle_length=1, shuffle=True, shuffle_buffer_size=10000, window_length=15, window_random=False,top=25,as_tuple=True,repeat=1):
...
def frontend_log_mel_spect(input, y_input=128, num_filts=32):
...
def build_model(frontend_mode, num_output_neurons=25, y_input=128, num_units=500, num_filts=16, batch_size=32):
...
if __name__== "__main__":
    ...
    train_ds,valid_ds,test_ds = generate_datasets_from_dir('/srv/data/mtt/tfrecords/log-mel-complete','log-mel-spectrogram')
    print("Dataset prepared")
    log_dir = os.getcwd()
    log_dir = os.path.join(os.path.expanduser(log_dir), 'mtt_log-mel-spectogram_stage_3',)
    ...
    model = build_model('log-mel-spectrogram')
    ...
    with open(os.path.join(directory,'mtt_stage_3_model_log-mel_test.txt'), 'w') as f:
