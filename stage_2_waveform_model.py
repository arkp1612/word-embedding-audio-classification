...
def _generate_datasets(tfrecords, audio_format, split=None, which_split=None, sample_rate=16000, num_mels=96, batch_size=64, block_length=1, cycle_length=1, shuffle=True, shuffle_buffer_size=10000, window_length=15, window_random=False,top=50,as_tuple=True,repeat=1):
...
def build_model(frontend_mode, num_output_neurons=50, y_input=96, num_units=500, num_filts=16, batch_size=64):
...
if __name__== "__main__":
    train_ds,valid_ds,test_ds = generate_datasets_from_dir('tfrecord','waveform')
    log_dir = os.getcwd()
    ...
    with open(os.path.join(directory,'stage_2_model_waveform_test.txt'), 'w') as f:
